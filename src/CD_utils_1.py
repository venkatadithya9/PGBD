        ...
        for bottleneck_name in bottlenecks:
            ...
            for c in tqdm(class_labels):
                ...
                def save_activation_hook(mod, inp, out):
                    ...
                    bn_activation = out
                X_c_dataloader = DataLoader(data, batch_size = bss, sampler = SubsetRandomSampler(inds))
                    ...
                    for f in range(opt.knn_k):
                        if c in proto_dic:
                            if ss_len == 4:
                                ...
                                proto_dic[c]['cluster_'+str(f)] = centers[f].reshape(ss[1])
...
def save_classwise_protos_delta(model, data, opt, prev_protos, bss = 35, use_knn_proto = True):
    model.eval()
    class_labels = [i for i in range(opt.num_class)]
    protos = {}
    named_layers = dict(model.named_modules())
            ...
            for c in tqdm(class_labels):
                ...
                if use_knn_proto and opt.knn_k != 1:
                    ...
                    for f in range(opt.knn_k):
                        if c in proto_dic:
                            if len(ss) == 4:
                                proto_dic[c]['cluster_'+str(f)] = (opt.delta)*centers[f].reshape(ss[1],ss[2],ss[3]) + (1-opt.delta)*(prev_proto_dic[c]['cluster_'+str(f)])
                            else:
                                proto_dic[c]['cluster_'+str(f)] = (opt.delta)*centers[f].reshape(ss[1]) + (1-opt.delta)*(prev_proto_dic[c]['cluster_'+str(f)])
                    ...
                    proto_dic[c] = (opt.delta)*torch.mean(acts,axis=0).unsqueeze(0).detach().cpu().numpy() + (1 - opt.delta)*(prev_proto_dic[c])
            protos[bottleneck_name] = (proto_dic)
    return protos
...
def save_classwise_protos_dino_mm(model, data, opt, bss = 35, use_knn_proto = True, upconvs = None, downconvs = None):
    model.eval()
    print("new ")
    # for param in model.parameters():
    #     param.requires_grad = False
    ...
    for bottleneck_name in bottlenecks:
        if len(upconvs) == 0 or len(downconvs) == 0:
            dino_acts_path = f"dino_activations_dinov2_{opt.s_name}_{opt.dataset}_{opt.attack_method}_{bottleneck_name[-2:]}"
            student_acts_path = f"student_activations_dinov2_{opt.s_name}_{opt.dataset}_{opt.attack_method}_{bottleneck_name[-2:]}"
...
def save_classwise_protos_dino_delta(model, data, opt , prev_protos, bss = 35, use_knn_proto = True):
    model.eval()
    print("new")
    # for param in model.parameters():
    #     param.requires_grad = False
...
def save_classwise_protos_dino_mm_delta(model, data, opt , prev_protos, bss = 35, use_knn_proto = True, upconvs = None, downconvs = None):
    model.eval()
    print("new")
    # for param in model.parameters():
    #     param.requires_grad = False
...
def get_dataset_normalization(dataset_name):
    ...
    if dataset_name == "cifar10":
        # from wanet
        dataset_normalization = (tfs.Normalize([0.4914, 0.4822, 0.4465], [0.247, 0.243, 0.261]))
    elif dataset_name == 'cifar100':
        '''get from https://gist.github.com/weiaicunzai/e623931921efefd4c331622c344d8151'''
        dataset_normalization = (tfs.Normalize([0.5071, 0.4865, 0.4409], [0.2673, 0.2564, 0.2762]))
    elif dataset_name == "mnist":
        dataset_normalization = (tfs.Normalize([0.5], [0.5]))
    elif dataset_name == 'tiny':
        dataset_normalization = (tfs.Normalize([0.4802, 0.4481, 0.3975], [0.2302, 0.2265, 0.2262]))
...
def get_dino_activations(dataloader, device, pca_components=64):
    # model = get_model(model_name = "dino", model_path = "./N3F/feature_extractor/ckpts/dino_vitbase8_pretrain.pth", device=device)
...
def save_acts(model, opt, train_data, train_dino_data, train_pois_data = None, train_dino_pois_data = None, only_student = False):
    ...
    for i in range(num_csets):
        ...
        for ind in range(num_classes):
            sel_list = set()
            sel_list.update(np.random.choice(list(indices[ind]), size = max(int(cset_len//num_classes), 1), replace = False).tolist())
            indices[ind] = indices[ind] - sel_list
    ...
    for bottleneck_name in bottlenecks:
        ...
        for i in range(num_csets):
            ...
            torch.save(acts_clean, f"student_activations_{opt.s_name}_{opt.dataset}_{opt.attack_method}_{bottleneck_name[-2:]}_clean{i}.pt")
            if not only_student:
                dino_acts_clean = get_dino_activations(clean_dino_dataloader, device)
                print("DINO ACTS SHAPE: ", dino_acts_clean.shape)
                pairs_lis[f"clean{i}"] = [("","")]
                torch.save(dino_acts_clean, f"dino_activations_{opt.s_name}_{opt.dataset}_{opt.attack_method}_{bottleneck_name[-2:]}_clean{i}.pt")

        if pois_set:
            pois_dataloader = DataLoader(pois_set, batch_size = 1)
            pois_dino_dataloader = DataLoader(train_dino_pois_data, sampler = SubsetRandomSampler(balanced_pois_inds), batch_size = 1)
            acts_pois, _ = get_activations(model, pois_dataloader, named_layers, bottleneck_name, device)
            torch.save(acts_pois, f"student_activations_{opt.s_name}_{opt.dataset}_{opt.attack_method}_{bottleneck_name[-2:]}_pois.pt")
            if not only_student:
                dino_acts_pois = get_dino_activations(pois_dino_dataloader, device)
                torch.save(dino_acts_pois, f"dino_activations_{opt.s_name}_{opt.dataset}_{opt.attack_method}_{bottleneck_name[-2:]}_pois.pt")
...
def save_synthetic_acts(model, opt, train_data, train_dino_data, load_existing = False):
    ...
    if load_existing:
        for f in glob.iglob(f"./trig_sets/synthetic_{opt.s_name}_{opt.attack_method}_{opt.dataset}*.png"):
            trig = Image.open(f)
        ...
        for g in range(5,100,5):
            for s in seeds:
                pattern = backdoor.generate(train_loader, opt.target_label,
                                        attack_size=opt.attack_size, gamma = g, seed = s)
                print(g, len(pattern))
                tf_to_img = tfs.ToPILImage()
                trig_img = tf_to_img(pattern)
                trig_img.save("./trig_sets/synthetic_" + opt.s_name+ "_" + opt.attack_method+ "_" + opt.dataset + "_g_" + str(g) + "_seed_" + str(s) + ".png")
