    ...
    for bneck in opt.bottlenecks:
        ...
        for c in range(opt.num_class):
            if c != opt.target_label:
                ...
                cav[c] = torch.zeros_like(get_proto(c, proto_dic).cuda())
                cav_dino[c] = torch.zeros_like(target_proto_dino)
...
def cd(opt):
    ...
    if opt.attack_method == "semantic" or opt.attack_method == "semantic_mask":
        ...
        if opt.dataset == "CIFAR10":
            ...
            if opt.all2all:
                test_bad_data = ImageFolder(
                    "../BackdoorBench/record/cifar10_preactresnet18_wanet_0_1_all2all/bd_test_dataset/",
                    transform=test_transforms,
                )
            ...
            test_bad_data = ImageFolder(
                "../BackdoorBench/record/tiny_preactresnet18_wanet_0_1",
    ...
    acts_classwise = None
    target_original = opt.target_label
    opt.update_gap = int(opt.epochs / opt.num_class) # cycle time calculation based on total number of epochs
    if opt.epochs < opt.num_class:
        print("Number of epochs < Number of classes in the target class.")
        exit
    for epoch in range(opt.epochs):
        ...
        opt.target_label = target_original
...
if __name__ == "__main__":
    ...
    if opt.attack_method == "semantic_tattoo":
        print("tattoo")
        opt.model = f"./weight/{opt.dataset}/{opt.s_name}-{opt.attack_method}1.pth.tar"
        print("Model selected:", opt.model)
