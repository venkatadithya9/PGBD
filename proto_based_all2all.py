...
from config import get_arguments, get_arguments_1, get_arguments_2, get_arguments_3, get_arguments_2_preact, get_arguments_3_preact, get_arguments_4_preact, get_arguments_signal_preact, get_arguments_wanet_preact
import torch
import numpy as np
    ...
    for bottleneck_name in opt.bottlenecks:
        for c in tqdm.tqdm(class_labels):
            ...
            def save_activation_hook(mod, inp, out):
                ...
                bn_activation = out
            X_c_dataloader = DataLoader(data, batch_size = 35, sampler = SubsetRandomSampler(inds))
            if dino == False:
...
def tsne_vis(opt, p, e, acts_classwise, acts_classwise_pois, epoch, seed=42):
    ...
    colors = ['#1f77b4', '#1f77b4', '#ff7f0e', '#ff7f0e', '#2ca02c', '#2ca02c', '#d62728', '#d62728', '#9467bd', '#9467bd', '#8c564b', '#8c564b', '#e377c2', '#e377c2', '#7f7f7f', '#7f7f7f', '#bcbd22', '#bcbd22', '#17becf', '#17becf']
    ...
    for i in range(opt.num_class-1,-1,-1):  
        if i != opt.target_label:
            plt.scatter(acts_classwise_tsne[i][:-1, 0], acts_classwise_tsne[i][:-1, 1], label=str(i), s = 10, c= colors[i])
            
        else:
            plt.scatter(acts_classwise_tsne[i][:, 0], acts_classwise_tsne[i][:, 1],label=str(i) + " (target)", c="grey", s = 12)
    ...
    acts_pois_tsne = np.vstack([acts_classwise_pois_tsne[c] for c in acts_classwise_pois])
    acts_pois_mean_tsne = np.mean(acts_pois_tsne, axis=0)
    
    plt.scatter(acts_pois_tsne[:,0], acts_pois_tsne[:,1], label= "poisoned", c="black", s = 1 ,marker = "^")
...
def inversion(args, model, target_label, train_loader, gamma = 10, seed = 1234):
    ...
    pattern = backdoor.generate(train_loader, target_label,
                                attack_size=args.attack_size, gamma = gamma, seed = seed)
...
def train_step_cd(opt, train_data, train_pois_data, train_loader, nets, optimizer, criterions, pairs_lis, protos, protos_pois, wtcavs, cav_dic, cav_dic_class):
    model = nets['model']
    if opt.eval_mode:
        model.eval()  
    else:
    ...
    for cur_iter, data in enumerate(train_loader):
        ...
        for bneck in opt.bottlenecks:
            ...
            for b in range(imgs.shape[0]):
                ...
                vec = None
                if opt.agg_cav:
                    dot = torch.dot(grad_[0].to(device).float().squeeze(0).flatten(),direc_mean.to(device).float().flatten())/torch.linalg.norm(direc_mean)
                else:
                    if gt[b].item() != opt.target_label:
                        vec = cav_dic_class[bneck][gt[b].item()].clone()
                    else:
                        vec = grad_[0]
                    dot = torch.dot(grad_[0].to(device).float().squeeze(0).flatten(),vec.to(device).float().flatten())/torch.linalg.norm(vec)
                dot_lis.append(dot)
                ...
                if opt.agg_cav:
                    unit_direc = direc_mean.to(device).float().flatten()/torch.linalg.norm(direc_mean)
                else:
                    unit_direc = vec.to(device).float().flatten()/torch.linalg.norm(vec)
                
                
                mse_dvec_lis.append(mse_loss(unit_grad, unit_direc))

                if 'cos' in opt.loss_type: ### Sanitiztion loss
                    if opt.agg_cav:
                        cos_ = cos(grad_[0].to(device).float().squeeze(0).flatten(),direc_mean.to(device).flatten().float())
                    else:
                        cos_ = cos(grad_[0].to(device).float().squeeze(0).flatten(),vec.to(device).flatten().float())
                    cos_lis.append(cos_)
            ...
            if opt.loss_type=='mse':
                ...
                if affect!=None:
                    if affect==False:
                        ...
                        tcav_loss = None                                
                        tcav_loss = criterion(dot_lis.unsqueeze(0).to(device), target.unsqueeze(0).to(device))
                        loss_ = -tcav_loss
        ...
        if opt.use_wandb:
            wandb.log({"train loss": loss_final, "gt_loss": gt_loss, "cav_loss":sum(cav_loss_lis)})
...
def get_cav(opt, protos, pairs_lis, protos_pois = None, upconvs = None, downconvs = None):
    ...
    for bneck in opt.bottlenecks:
        ...
        if len(upconvs) == 0 or len(downconvs) == 0:
            ...
            upconv_m, downconv_m =  map_activation_spaces(dino_acts_path, student_acts_path, pairs_lis, 150, opt)
        ...
        for c in range(opt.num_class):
            # print(opt.distill)
            if opt.distill == True and opt.dino_acts == False and opt.dino_acts_mm == False: # Because we don't need upnconv in both cases
        ...
        for c in range(opt.num_class):
            if c != opt.target_label:
                if protos_pois:
                    ...
                    cav_dino[c] = class_protos[c] - class_protos[(c+1)%opt.num_class]
                else:
...
def get_cav_delta(opt, protos, pairs_lis, prev_cav_dic, prev_cav_dic_agg, protos_pois = None, upconvs = None, downconvs = None):
    cav_dic = dict()
    cav_dic_agg = {}
    if upconvs == None or downconvs == None:
        upconvs = {}
    ...
    for bneck in opt.bottlenecks:
        ...
        for c in range(opt.num_class):
            if c != opt.target_label:
                if protos_pois:
                    ...
                    cav_dino[c] = class_protos[c] - target_proto_dino
                if opt.distill:
                    cav[c] = (opt.delta)*downconv_m(cav_dino[c]) + (1 - opt.delta)*prev_cav[c]
                else:
        ...
        if opt.distill:
            ...
            cav_agg = cav_dino_agg.cuda()
        cav_dic[bneck] = cav
        cav_dic_agg[bneck] = (opt.delta)*cav_agg + (1-opt.delta)*prev_cav_dic_agg[bneck]
    return cav_dic, cav_dic_agg

def cd(opt):
    print(opt.num_class)
    model = select_model(dataset=opt.dataset,
                           model_name=opt.s_name,
                           pretrained=True,
                           pretrained_models_path=opt.model,
                           n_classes=opt.num_class, ml_mmdr = opt.ml_mmdr, all2all = opt.all2all).to(opt.device)
    ...
    if opt.attack_method == "semantic" or opt.attack_method == "semantic_mask":
        test_transforms =  tfs.Compose([tfs.ToTensor(), tfs.Resize((64,64)), 
                                        tfs.Normalize(torch.tensor([0.6668, 0.5134, 0.4482]), 
                                                      torch.tensor([0.0691, 0.0599, 0.0585])),
                                        tfs.RandomHorizontalFlip()])
        semantic = ROF(pratio = opt.inject_portion)
        if opt.trigger_type == "semanticMaskTrigger":
            semantic = ROF(pratio = opt.inject_portion, base_pth = "./data/ROF/data1", base_pth_pois = "./data/ROF/data_masked")
        ...
        test_transforms = tfs.Compose([tfs.ToTensor(), 
                                       tfs.Resize(size=(32, 32), max_size=None, antialias=None)
                                      ])
        test_bad_data = None
        if opt.dataset == "CIFAR10":
            test_bad_data = ImageFolder("../BackdoorBench/record/cifar10_preactresnet18_wanet_0_1/bd_test_dataset/", transform=test_transforms)
            if opt.all2all:
                test_bad_data = ImageFolder("../BackdoorBench/record/cifar10_preactresnet18_wanet_0_1_all2all/bd_test_dataset/", transform=test_transforms)
        elif opt.dataset == "CIFAR100":
            test_bad_data = ImageFolder("../BackdoorBench/record/cifar100_preactresnet18_wanet_0_1/bd_test_dataset/", transform=test_transforms)
        elif opt.dataset == "gtsrb":
            test_bad_data = ImageFolder("../BackdoorBench/record/gtsrb_preactresnet18_wanet_0_01/bd_test_dataset/", transform=test_transforms)
    ...
    g = 10
    s = 1234
    trig_path = f"./trig_sets/synthetic_{opt.s_name}_{opt.attack_method}_{opt.dataset}_g_{g}_seed_{s}.png"
    pattern = None
    try:
    ...
    if opt.cav_type == "synth":
        train_pois_data = get_backdoor_loader(opt, shuffle=False, without_loader = True, use_available= True, pattern=pattern)
        train_dino_pois_data = get_backdoor_loader(opt, shuffle=False, without_loader = True, use_available= True, pattern=pattern, dino=True)
    if opt.cav_type == "gt":
        train_gt_pois_data = get_backdoor_loader(opt, shuffle=False, without_loader= True, use_available= True)
        train_dino_gt_pois_data = get_backdoor_loader(opt, shuffle=False, without_loader= True, use_available= True, dino=True)
    ...
    if os.path.exists(dino_acts_path + "_clean0.pt") and os.path.exists(student_acts_path+ "_clean0.pt"):
        pass
    else: 
        if opt.cav_type == "proto":
            num_csets, pairs_lis = save_acts(model, opt, train_data, train_dino_data)
        elif opt.cav_type == "synth":
            num_csets, pairs_lis = save_acts(model, opt, train_data, train_dino_data, train_pois_data, train_dino_pois_data)
        elif opt.cav_type == "gt    ":
            num_csets, pairs_lis = save_acts(model, opt, train_data, train_dino_data, train_gt_pois_data, train_dino_gt_pois_data)

    protos = save_classwise_protos(model, train_data, opt) ################################## *******************************
    protos_pois = None
    ...
    if opt.dino_acts:
        ...
        if opt.cav_type == "synth":
            protos_pois_dino = save_classwise_protos_dino(model, train_dino_pois_data, opt)
        if opt.cav_type == "gt":
            protos_gt_pois_dino = save_classwise_protos_dino(model, train_dino_gt_pois_data, opt)
    elif opt.dino_acts_mm:
        protos_dino, upconvs, downconvs = save_classwise_protos_dino_mm(model, train_data, opt)
        if opt.cav_type == "synth":
            protos_pois_dino, _, _ = save_classwise_protos_dino_mm(model, train_pois_data, opt, upconvs, downconvs)
        if opt.cav_type == "gt":
            protos_gt_pois_dino, _, _ = save_classwise_protos_dino_mm(model, train_gt_pois_data, opt, upconvs, downconvs)
    ...
    if opt.dino_acts:
        ...
        cav_dic, cav_dic_agg = get_cav(opt, protos_dino, pairs_lis, protos_pois_dino, upconvs, downconvs)
    else:
    ...
    if opt.use_wandb:
        wandb.init(project="Concept Based Detriggering", config={
            "s_name": opt.s_name,
    ...
    for epoch in range(opt.epochs):
        ...
        if opt.update_cav and epoch%opt.update_gap == 0 and opt.update_gap_iter == -1:
            if opt.weight_proto:
                if opt.dino_acts:
                    ...
                    protos_dino = save_classwise_protos_dino_delta(model, train_dino_data, opt, protos_dino)
                elif opt.dino_acts_mm:
                    protos_dino, upconvs, downconvs = save_classwise_protos_dino_mm_delta(model, train_data, opt, protos_dino)
                else:
                    protos = save_classwise_protos_delta(model, train_data, opt, protos)
                if opt.cav_type == "synth":
                    if opt.dino_acts:
                        protos_pois_dino = save_classwise_protos_dino_delta(model, train_dino_pois_data, opt, protos_pois_dino)
                    elif opt.dino_acts_mm:
                        protos_pois_dino, _, _ = save_classwise_protos_dino_mm_delta(model, train_pois_data, opt, protos_pois_dino, upconvs, downconvs)
                    else:    
                        protos_pois = save_classwise_protos_delta(model, train_pois_data, opt, protos_pois)
                elif opt.cav_type == "gt":
                    if opt.dino_acts:
                        protos_pois_dino = save_classwise_protos_dino_delta(model, train_dino_gt_pois_data, opt, protos_gt_pois_dino)
                    elif opt.dino_acts_mm:
                        protos_pois_dino, _, _ = save_classwise_protos_dino_mm_delta(model, train_gt_pois_data, opt, protos_pois_dino, upconvs, downconvs)
                    else:    
                        protos_pois = save_classwise_protos_delta(model, train_gt_pois_data, opt, protos_gt_pois)
            else:                    
                delta = opt.delta
                opt.delta = 1
                if opt.dino_acts:
            ...
            if opt.weight_cav:
                if opt.dino_acts:
                    cav_dic, cav_dic_agg = get_cav_delta(opt, protos_dino, pairs_lis, cav_dic, cav_dic_agg, protos_pois_dino)
                elif opt.dino_acts_mm:
                    cav_dic, cav_dic_agg = get_cav_delta(opt, protos_dino, pairs_lis, cav_dic, cav_dic_agg, protos_pois_dino, upconvs, downconvs)
                else:
                    cav_dic, cav_dic_agg = get_cav_delta(opt, protos, pairs_lis, cav_dic, cav_dic_agg, protos_pois)
            else:
                delta = opt.delta
                opt.delta = 1
                if opt.dino_acts:
        ...
        train_step_cd(opt, train_data, train_pois_data, train_loader, nets, optimizer, criterions, pairs_lis, protos, protos_pois, wtcavs, cav_dic_agg, cav_dic)
...
if __name__ == "__main__":            
    ...
    opt.all2all = True # changes the way PAVs are computed

    if opt.all2all:
        opt.target_type = "all2all"
    ...
    if opt.attack_method == "semantic_mask":
        ...
        opt.model = f"./weight/{opt.dataset}/{opt.s_name}-{opt.attack_method}_gen.pth.tar"
