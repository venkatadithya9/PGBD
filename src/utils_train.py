...
class EncoMapVGG19BN(torch.nn.Module):
    '''
    Mainly for mapping to match dims
    '''
    def __init__(self):
        super(EncoMapDMNISTNN, self).__init__()
        self.conv = nn.Conv2d(64,512,(4,4),stride=1)
       
    def forward(self,x):
        x = self.conv(x)
        return x

class DecoMapVGG19BN(torch.nn.Module):
    '''
    Mainly for mapping to match dims
    '''
    def __init__(self):
        super(DecoMapDMNISTNN, self).__init__()
        self.conv = nn.ConvTranspose2d(512,64,(4,4),stride=1)
...
def get_model_save_name(opt):
    ...
    k = ["pair_vals","num_imgs","lr","regularizer_rate","wtcav","batch_size","cur_proto_mean_wt","use_proto","use_cdepcolor","use_precalc_proto","update_proto","train_from_scratch","use_knn_proto","class_wise_training"]
    ks = {"pair_vals":"p","num_imgs":"nimg","lr":"lr","regularizer_rate":"rr","wtcav":"wtcav", "batch_size":"bs","cur_proto_mean_wt":"pwt","use_proto":"upr","use_cdepcolor":"cd","use_precalc_proto":"precalc","update_proto":"up","train_from_scratch":"scratch","use_knn_proto":"uknn","class_wise_training":"cwt"}
    dic = opt.__dict__
    for p in k:
...
def get_vanilla_model(model_type):
    ...
    if model_type == 'decoymnist':
        ...
        model.cuda()
        shape = (28,28)
        model.load_state_dict(torch.load('dep/mnist/DecoyMNIST/orig_model_decoyMNIST_.pt'))

    if model_type == 'colormnist':
        model = MNISTColorNet()
        model.cuda()
        shape = (28,28)
        model.load_state_dict(torch.load('dep/mnist/ColorMNIST/orig_model_colorMNIST.pt'))
...
def get_model_and_data(model_type, opt, kwargs):
    ...
    if model_type == 'decoymnist':
        ...
        if not opt.train_from_scratch:
            ...
            # model.load_state_dict(torch.load('/media/Data2/avani.gupta/best_checkpoints/acc98.98best47decoymnistdmnistpair_vals5num_imgs150lr0.58regularizer_rate0.3wtcav0.93batch_size256per_proto_mean_wt0.35use_proto0use_cdepcolor1use_precalc_proto1update_proto1train_from_scratch1use_knn_proto0class_wise_training0final_run'))
            model.load_state_dict(torch.load('dep/mnist/DecoyMNIST/orig_model_decoyMNIST_.pt'))
        bottleneck_name = 'conv2'
